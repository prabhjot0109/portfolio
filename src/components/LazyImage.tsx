...
const LazyImage: React.FC<LazyImageProps> = ({
  ...
  return (
    <div ref={imgRef} className={`relative overflow-hidden ${className}`}>
      ...
      {showBlurBackground && objectFit === "contain" && isLoaded && (
        <div
          className="absolute inset-0 z-0 scale-110 blur-2xl opacity-50"
          style={{
      ...
      {isInView && (
        <img
          ...
          onLoad={handleLoad}
          onError={handleError}
          className={`relative z-10 w-full h-full object-${objectFit} ${className}`}
