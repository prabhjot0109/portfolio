...
>(({ project, index }, ref) => {
  return (
    <Dialog>
      ...
      >
        <motion.div
          className="group relative h-full bg-card/90 dark:bg-card/20 backdrop-blur-xl border border-border dark:border-white/10 rounded-2xl overflow-hidden hover:border-primary/20 transition-all duration-500 flex flex-col shadow-lg hover:shadow-[0_14px_28px_-14px_rgba(15,23,42,0.35)] dark:hover:shadow-[0_14px_28px_-14px_rgba(148,163,184,0.24)]"
          whileHover={{
            y: -12,
            scale: 1.02,
          }}
          ...
          <div className="relative aspect-video overflow-hidden border-b border-border/10 group-hover:border-primary/20 transition-colors duration-500">
            ...
            {project.title === "Sentient AI" && (
              <div className="absolute top-4 right-4 z-20">
                <Badge
                  className="bg-blue-500/90 text-white border-none shadow-lg backdrop-blur-md"
                >
                  Ongoing
                </Badge>
              </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {project.demoUrl !== "#" ? (
                    <motion.a
                      ...
                      whileTap={{ scale: 0.95 }}
                    >
                      <Globe className="w-3.5 h-3.5 transition-transform duration-200 group-hover/demo:rotate-12" />
                      Demo
                      <ExternalLink className="w-2.5 h-2.5 opacity-50 group-hover/demo:opacity-100 transition-opacity duration-200" />
                    </motion.a>
                  ) : (
                    <Popover>
                      ...
                      <PopoverContent
                        side="top"
                        align="start"
                        className="w-64 p-0 border border-border/60 bg-card/95 backdrop-blur-xl shadow-xl rounded-xl overflow-hidden"
                        sideOffset={8}
                      >
                        <div className="p-4 space-y-3">
                          <div className="flex items-start gap-3">
                            <div className="mt-0.5 p-1.5 rounded-lg bg-muted/60 border border-border/50">
                              <MonitorX className="w-4 h-4 text-muted-foreground" />
                            </div>
                            <div>
                              <p className="text-xs font-semibold text-foreground mb-0.5">No Live Demo</p>
                              <p className="text-[11px] text-muted-foreground leading-relaxed">
                                This is an app or desktop software — a live web demo isn't available.
                              </p>
                            </div>
                          </div>
                          <div className="pt-1 border-t border-border/40">
                            <p className="text-[10px] text-muted-foreground/70 mb-2">Explore the source code instead</p>
                            <a
                              href={project.githubUrl}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="no-hover inline-flex items-center gap-2 w-full px-3 py-2 rounded-lg text-xs font-medium
                                bg-foreground text-background
                                transition-all duration-200
        ...
        >
          <div className="w-full overflow-x-auto scrollbar-hidden md:overflow-visible pb-2 -mb-2">
            <div className="flex justify-start md:justify-center min-w-max md:min-w-0">
              <div className="inline-flex p-1 rounded-full bg-muted/50 backdrop-blur-sm border border-border/50 gap-0.5 md:gap-1">
                {categories.map((category) => (
                  <button
                    ...
                    className={`px-2.5 sm:px-3 md:px-5 py-1.5 sm:py-2 md:py-2.5 rounded-full text-[11px] sm:text-xs md:text-sm font-medium transition-all duration-300 whitespace-nowrap ${activeCategory === category.id
