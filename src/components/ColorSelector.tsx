  ...
  return (
    <div className="relative">
      <Button
        ...
        className="hover:bg-accent/10 relative h-8 w-8 p-1 rounded-full border border-muted-foreground/20 hover:border-accent transition-all duration-200"
        aria-label="Select portfolio color theme"
        aria-expanded={isOpen}
      >
        {selectedColor === "none" ? (
          <Circle className="w-4 h-4 text-foreground" strokeWidth={2} />
        ) : (
          <div
            className="w-4 h-4 rounded-full border border-background shadow-sm"
            style={{
              backgroundColor: `hsl(${
                colors.find((c) => c.name === selectedColor)?.primary ||
                "var(--foreground)"
              })`,
            }}
          />
      ...
      {isOpen && (
        <div
          className="absolute top-full right-0 mt-2 p-3 rounded-xl border border-border/50 shadow-xl z-50
                     bg-background/98 backdrop-blur-xl transition-all duration-200 animate-fade-in"
          style={{ minWidth: "60px" }}
        >
          <div className="grid grid-cols-1 gap-2">
            {colors.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full border-2 transition-all duration-200 hover:scale-110 hover:shadow-lg flex items-center justify-center
                           ${
                             selectedColor === color.name
                               ? "border-foreground shadow-lg scale-110 ring-2 ring-accent/30"
